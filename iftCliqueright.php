						 ...
						 $proid = $get_userid['proid'];
							...
							foreach ($json as $value) {
								...
								if($get_pro)
								{
								   ...
								   {
					?>
						<li onclick="get_product('<?php echo $product['proid']; ?>')"><img src="<?php get_image($product['image_code']); ?>" alt="<?php echo $allproducts['pro_name']; ?>" width="92" height="92" /></li>
					<?php  } 
			   } 
			}
		  }
...
function get_product(pid)
{
	...
	var proId = pid;
	$.ajax({
		url: '<?php echo ru;?>process/get_filterdiftclique.php?picID='+proId,
		type: 'get', 
